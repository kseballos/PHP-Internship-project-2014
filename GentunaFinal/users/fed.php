...
<head>
	  ...
	  <div class="row">
          <div class="col-lg-12">
            <div class="widget-container fluid-height clearfix">
              <div class="heading">
                <i class="icon-table"></i>Job Orders
				<a href="addjoborder.php"><button class="btn btn-sm btn-primary-outline pull-right" type="submit">
 ...
 <?php
									...
									$sql1 =" SELECT hourscompleted,estimateddate,enddate,projectname,datetime , username ,estimatedcost ,username, cerpnum, controlnum, departmentid, statusid, datereq, prioritynumber ,rateequivalent
									FROM joborderdetails order by username DESC " ;
					...
					echo    "<td >" .$rows['hourscompleted'] . "</td>";
						if($rows['username'] == $username1 && $stat == 'APPROVED'){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i><i class='glyphicon glyphicon-ok' title='Not Allowed to Acknowledge Job Order' ></i></a></div>"  ; 
					  echo "</tr>";
							}
					else if($rows['username'] == $username1 && $stat == 'Open'){
					echo "<td class='actions'><div class='action-buttons'><a class='table-actions' href='editjoborder.php?p=$prio'><i class='icon-pencil' 'title='Edit Request' ></i></a><i class='glyphicon glyphicon-ok' title='Not allowed to Acknowledge Job Order'></i></a></div>"  ; 
					  echo "</tr>";
							}
					else if($rows['username'] == $username1 && $stat == 'HOLD'){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i><i class='glyphicon glyphicon-ok' title='Not Allowed to Acknowledge Job Order' ></i></a></div>"  ; 
					  echo "</tr>";
							}
					else if($rows['username'] == $username1 && $stat == 'ON-GOING'){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i><i class='glyphicon glyphicon-ok' title='Not Allowed to Acknowledge Job Order' ></i></a></div>"   ; 
					  echo "</tr>";
							}
					else if($rows['username'] == $username1 && $stat == 'CANCELLED'){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i><i class='glyphicon glyphicon-ok' title='Not Allowed to Acknowledge Job Order' ></i></a></div>"  ; 
					  echo "</tr>";
							
							}
							else if($rows['username'] == $username1 && $stat == 'ACKNOWLEDGED'){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i></a><i class='glyphicon glyphicon-ok' title='You have already Acknowledged this Job Order'</i></a></div>"  ;
					  echo "</tr>";
							}
								else if($rows['username'] == $username1 && $stat == 'COMPLETED' ){
					echo "<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'></i><a class='table-actions' href='completed1.php?p=$prio'><i class='glyphicon glyphicon-ok' title='Not allowed to Complete Job Order' ></i></a></div>"  ;
					  echo "</tr>";
							}
					else {
					  
					  echo 
					"<td class='actions'><div class='action-buttons'><i class='icon-pencil' title='Not allowed to Change Details'> </i><i class='glyphicon glyphicon-ok' title='Not allowed to Acknowledge Job Order'></i> </div>" ;

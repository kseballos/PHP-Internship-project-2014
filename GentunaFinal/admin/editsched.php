...
<head>
  ...
  <body>
	...
	$sql= "UPDATE scheduler SET username='$user', first='$first', departmentid='$deptid', last='$last' where username ='$p' ";
	$insert1 = mysql_query($sql);
	$sql1= "UPDATE loginacc SET username='$user', password='$pass' where username ='$p' ";
	$insert2 = mysql_query($sql1);
	
if($insert1){
echo "<script type=\"text/javascript\">window.location.href='managesched.php' </script>";
}
else if($insert2){
echo "<script type=\"text/javascript\">window.location.href='managesched.php' </script>";
}
